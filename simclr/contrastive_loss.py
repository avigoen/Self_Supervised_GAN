...
def sim(z_i, z_j):
    ...
    norm_dot_product = norm_dot_product / (norm_i * norm_j)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_positive = sim(z_k.unsqueeze(0), z_k_N.unsqueeze(0))  # scalar value
        neg_pairs = torch.cat(
            [out[: k + N], out[k + N + 1 :]], dim=0
        )  # exclude k and k+N
        sim_negative = sim(z_k.unsqueeze(0), neg_pairs)  # [2N-2] vector

        numerator = torch.exp(sim_positive / tau)
        denominator = torch.sum(torch.exp(sim_negative / tau))
...
def sim_positive_pairs(out_left, out_right):
    ...
    dot_product = torch.sum(out_left * out_right, dim=1, keepdim=True)  # Shape: [N, 1]
    norm_left = torch.norm(out_left, dim=1, keepdim=True)  # Shape: [N, 1]
    norm_right = torch.norm(out_right, dim=1, keepdim=True)  # Shape: [N, 1]
    norm_product = norm_left * norm_right  # Shape: [N, 1]
    pos_pairs = dot_product / norm_product  # Shape: [N, 1]
...
def compute_sim_matrix(out):
    ...
    dot_product = torch.matmul(out, out.t())  # Shape: [2N, 2N]
    norm = torch.norm(out, dim=1, keepdim=True)  # Shape: [2N, 1]
    norm_product = torch.matmul(norm, norm.t())  # Shape: [2N, 2N]
    sim_matrix = dot_product / norm_product  # Shape: [2N, 2N]
...
def simclr_loss_vectorized(out_left, out_right, tau, device="cuda"):
    ...
    numerator = torch.exp(sim_positive / tau)  # [N, N]
    numerator = numerator.sum(dim=1, keepdim=True)  # [N, 1]
